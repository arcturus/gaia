...
function processFiles(list, context, stagePath) {
  utils.log('preprocessor', 'My options are', JSON.stringify(context));
  list.forEach((fileName) => {
    fileName.unshift(stagePath);
    utils.log('preprocessor', fileName);
    pp.preprocessFileSync(fileName, fileName, context);
  });
}
...
exports.execute = function(options, flag, list) {
  ...
  processFiles(list.process, options, stagePath);
