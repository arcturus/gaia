...
var UIManager = {
  ...
  init: function ui_init() {
    ...
    // Bind any autogenerated buttons for sim import
    var importButtons = document.querySelectorAll(
      '#import_contacts button[data-id]');
    for (var i = 0; i < importButtons.length; i++) {
      importButtons[i].addEventListener('click', this);
    }
  ...
  handleEvent: function ui_handleEvent(event) {
    var id = event.target.id || event.target.dataset.id;
    switch (id) {
      // SIM
      ...
      case 'unlock-sim-button':
        ...
        break;
      case 'sim-import':
        window.setTimeout(SimManager.importContacts.bind(
          SimManager, event.target), 0);
        break;
  ...
  getSimImportButton: function ui_gsib(iccId) {
    return document.querySelector('[data-iccid="' + iccId + '"]');
  },

  getSimMessage: function ui_gsm(iccId) {
    var id = 'no-sim-' + iccId;
